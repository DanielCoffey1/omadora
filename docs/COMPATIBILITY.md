...
2. Resolve the reproduced QEMU/virtio suspend/resume hang. The GDM/Hyprland startup crash was traced to an inactive login session and fixed; 15 repeated logins passed across three VMs, including deliberately delayed launches. Expand the passing desktop/password/recovery checks to an untouched Workstation ISO installation and physical GPUs. The GTK file chooser's dark styling is fixed; complete application theme parity and multi-monitor testing remain. The existing fixture uses Workstation packages on the official Fedora Cloud image; it exercises both GDM autologin and actual lock-screen password authentication with SELinux enforcing.
