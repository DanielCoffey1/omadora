...
| Package pickers / completion | Real repository and installed RPM inventories filtered and multi-selected; web-app terminal closed with Enter | Offline repository behavior and every possible DNF dependency transaction |
| Recording / reminders | Region-picker cancellation, silent/desktop-audio/microphone recordings saved and decoded; indicator and menu captures; reminder timer creation/clear | Physical audio quality, multi-monitor behavior and GPU performance; webcam and mixed audio are not implemented |
